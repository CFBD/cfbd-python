...
class AdvancedBoxScoreTeams(BaseModel):
    ...
    """
    rushing_advanced: conlist(TeamRushingGame) = Field(default=..., alias="rushingAdvanced", description="Enriched rushing, separate from the legacy rushing section.")
    passing: conlist(TeamPassingGame) = Field(default=..., description="Enriched offense/defense passing; empty when no qualifying rows exist.")
    ...
    __properties = ["rushingAdvanced", "passing", "fieldPosition", "scoringOpportunities", "havoc", "rushing", "explosiveness", "successRates", "cumulativePpa", "ppa"]
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in rushing_advanced (list)
        _items = []
        if self.rushing_advanced:
            for _item in self.rushing_advanced:
                if _item:
                    _items.append(_item.to_dict())
            _dict['rushingAdvanced'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in passing (list)
        _items = []
        if self.passing:
            for _item in self.passing:
                if _item:
                    _items.append(_item.to_dict())
    ...
    def from_dict(cls, obj: dict) -> AdvancedBoxScoreTeams:
        ...
        _obj = AdvancedBoxScoreTeams.parse_obj({
            "rushing_advanced": [TeamRushingGame.from_dict(_item) for _item in obj.get("rushingAdvanced")] if obj.get("rushingAdvanced") is not None else None,
            "passing": [TeamPassingGame.from_dict(_item) for _item in obj.get("passing")] if obj.get("passing") is not None else None,
